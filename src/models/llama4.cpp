...
std::unique_ptr<llm_graph_context> llama_model_llama4::build_arch_graph(const llm_graph_params & params) const {
    ...
    // (chunked) sliding window per-layer in the paged-attention kernel, so route
    // it through the non-ISWA graph (single mctx + paged_attn); the ISWA graph
    // assumes a llama_kv_cache_iswa_context and would crash. Per-layer RoPE lives
    // in the shared graph body, so it is preserved on the non-ISWA path.
